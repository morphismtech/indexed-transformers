...
module Control.Monad.Trans.Indexed.Free
  ...
  , IxFilterable, IxerMaybe (IxerMaybe)
  ) where
...
type IxFilterable f = forall i j. Filterable (f i j)

data IxerMaybe f i j x where
  IxerMaybe :: (x -> Maybe y) -> f i j x -> IxerMaybe f i j y
instance Functor (IxerMaybe f i j) where
  fmap g (IxerMaybe f x) = IxerMaybe (fmap g . f) x
instance Filterable (IxerMaybe f i j) where
  mapMaybe g (IxerMaybe f x) = IxerMaybe (g <=< f) x
